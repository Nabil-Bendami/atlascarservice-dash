...
set
  is_suspended = true,
  is_blocked = coalesce(is_blocked, true)
...
create index if not exists agencies_public_visibility_state_idx
  on public.agencies(status, is_verified, is_suspended, is_blocked);
...
select
  ...
  coalesce(a.is_suspended, false) as suspended
...
using (
  status = 'active'
  and availability is distinct from false
  and exists (
    select 1
    from public.agencies a
...
using (
  exists (
    ...
    where c.id = car_images.car_id
      and c.status = 'active'
      and c.availability is distinct from false
      and a.status = 'active'
      and a.is_verified = true
      and coalesce(a.is_blocked, false) = false
      and coalesce(a.is_suspended, false) = false
